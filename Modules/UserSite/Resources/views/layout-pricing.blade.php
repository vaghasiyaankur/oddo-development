                                ...
                                <form action="" class="form-room-type">
                                    ...
                                    <div class="p-form-select pt-3">
                                        ...
                                        <input type="hidden" class="hotelId"
                                            value="{{ isset($hotel) ? $hotel->UUID : '' }}">
                                        <input type="hidden" class="roomId"
                                            value="{{ isset($roomDetail) ? $roomDetail->UUID : '' }}">
                                        <select class="form-select w-md-50 room_type ">
                                            <option value="">Please Select</option>
                                            @foreach ($room_types as $room_type)
                                                <option value="{{ $room_type->id }}"
                                                    {{ isset($roomDetail) && $room_type->id == $roomDetail->room_type_id ? 'selected' : '' }}>
                                    ...
                                    <div class="layout-room-name d-flex d-none">
                                        <div class="p-form-select pt-3  w-50">
                                            ...
                                            <label for="" class="form-label label-heading">This is the name guests
                                                will see
                                                on the website</label>
                                        </div>
                                        <div class="contact-number-inner w-50 ps-4 pt-3">
                                            <label for="" class="form-label label-heading ">Custom name
                                                (optional)</label>
                                            <input type="tel" class="form-control custom-from-control custom_name"
                                                value="{{ isset($roomDetail) ? $roomDetail->custom_name_room : '' }}">
                                            <label for="" class="form-label label-heading">Create an optional,
                                                custom name
                                    ...
                                    <div class="p-form-select pt-3">
                                        ...
                                        <select class="form-select w-md-50 smoking_area">
                                            <option value="n-smoking"
                                                {{ isset($roomDetail) && $roomDetail->smoking_policy == 'n-smoking' ? 'selected' : '' }}>
                                                Non-smoking</option>
                                            <option value="smoking"
                                                {{ isset($roomDetail) && $roomDetail->smoking_policy == 'smoking' ? 'selected' : '' }}>
                                                smoking</option>
                                            <option value="b-smoking"
                                                {{ isset($roomDetail) && $roomDetail->smoking_policy == 'b-smoking' ? 'selected' : '' }}>
                                                I have both smoking and non-smoking options for this room type</option>
                                        </select>
                                    </div>
                                    <div class="total-room-layout pt-3">
                                        <label for="" class="form-label label-heading ">Number of room(of this
                                            type)</label>
                                        <input type="number" class="form-control layout-totalroom number_of_room"
                                            value="{{ isset($roomDetail) ? $roomDetail->number_of_room : '' }}">
                                    </div>
                                </form>
                            </div>
                            ...
                            <div class="form-info-box mt-3 bed_option_div">
                                <form action="" class="form-bedoption">
                                    ...
                                    <div class="bed-option-title d-flex">
                                        <h5 for="" class="form-label label-heading">What kind of bed are available
                                            in this
                                            room?</h5>
                                            ...
                                            @if (isset($hotelBeds) && $hotelBeds->count() > 0)
                                                ...
                                                <select class="form-select w-50 bed_size bed_size_1 select-bed">
                                                    @foreach ($beds as $bed)
                                                        @foreach ($hotelBeds as $hotelBed)
                                                            <option value="{{ $bed->id }}"
                                                                {{ isset($hotelBed) && $bed->id == $hotelBed->bed_id ? 'selected' : '' }}>
                                                                {{ $bed->bed_type }} / {{ $bed->bed_size }}</option>
                                                        @endforeach
                                                    @endforeach
                                                </select>
                                                ...
                                                <select class="form-select c-form-select number_of_bed number_of_bed_1">
                                                    ...
                                                    <option value="1"
                                                        {{ isset($hotelBed) && $hotelBed->no_of_bed == '1' ? 'selected' : '' }}>
                                                        1</option>
                                                    <option value="2"
                                                        {{ isset($hotelBed) && $hotelBed->no_of_bed == '2' ? 'selected' : '' }}>
                                                        2</option>
                                                    <option value="3"
                                                        {{ isset($hotelBed) && $hotelBed->no_of_bed == '3' ? 'selected' : '' }}>
                                                        3</option>
                                                </select><br><br>
                                            @else
                                                ...
                                                <select class="form-select w-50 bed_size bed_size_1 select-bed">
                                                    @foreach ($beds as $bed)
                                                        <option value="{{ $bed->id }}"
                                                            {{ isset($roomDetail) && $bed->id == $roomDetail->room_type_id ? 'selected' : '' }}>
                                                            {{ $bed->bed_type }} / {{ $bed->bed_size }}</option>
                                                    @endforeach
                                                </select>
                                                <span class="px-4"><i class="fa-solid fa-xmark"></i></span>
                                    ...
                                    <div class="pannel-add-another py-3">
                                        <a href="javascript:;" class="para-fs-14" id="p_add_another"><i
                                                class="fa-solid fa-circle-plus purple"></i> <span class="purple">Add
                                                another bed</span></a>
                                    </div>
                                    <div class="bed-option-guest-number">
                                        <label for="" class="form-label label-heading">How many guests can stay in
                                            this
                                            room?</label>
                                        <div class="total-room-layout ">
                                            <input type="number"
                                                class="form-control layout-totalroom d-inline number_of_guest"
                                                value="{{ isset($roomDetail) ? $roomDetail->guest_stay_room : '1' }}">
                            ...
                            <div class="form-info-box mt-3">
                                <form action="" class="form-roomsize ">
                                    ...
                                    <div class="input-group mb-3 total-room-layout w-50">
                                        <input type="number" class="form-control custom-from-control room_size"
                                            placeholder="0"
                                            value="{{ isset($roomDetail) ? $roomDetail->room_size : '' }}">
                                        <select class="form-select room_size_feet">
                                            <option value="s-meter"
                                                {{ isset($roomDetail) && $roomDetail->room_cal_type == 's-meter' ? 'selected' : '' }}>
                                                square meters</option>
                                            <option value="s-feet"
                                                {{ isset($roomDetail) && $roomDetail->room_cal_type == 's-feet' ? 'selected' : '' }}>
                                                square feet</option>
                                        </select>
                                    </div>
                                </form>
                            ...
                            <div class="form-info-box mt-3">
                                <form action="" class="form-bathroom-part">
                                    ...
                                    <div class="bathroom-title pt-3">
                                        <label for="" class="form-label label-heading ">Is the bathroom
                                            private?</label>
                                    </div>
                                    <div class="amenities-raido-btn">
                                        <label class="form-check-label me-3" for="yes" style="display: initial;">
                                            <div class="form-check form-check-inline bathroom-radio me-0">
                                                <input class="form-check-input bathroom_private" type="radio"
                                                    name="bathroom_private" id="yes" value="yes"
                                                    {{ isset($roomDetail) && $roomDetail->bathroom_private == 'yes' ? 'checked' : '' }}>
                                                Yes
                                            </div>
                                        </label>
                                        <label class="form-check-label me-3" for="no" style="display: initial;">
                                            <div class="form-check form-check-inline bathroom-radio" me-0>
                                                <input class="form-check-input bathroom_private" type="radio"
                                                    name="bathroom_private" id="no" value="no"
                                                    {{ isset($roomDetail) && $roomDetail->bathroom_private == 'no' ? 'checked' : '' }}>
                                                No
                                            </div>
                                        </label>
                                    </div>
                                    ...
                                    <div class="p-form-heading d-flex">
                                        <div class="bathroom-second-title pt-3">
                                            <label for="" class="form-label label-heading ">Which bathroom items
                                                are avaliable in this room?</label>
                                        </div>
                                    </div>
                                    <div class="bathroom-item-list ">
                                        <div
                                            class="bathroom-item-check d-flex flex-wrap align-items-center justify-content-between">
                                            <input type="hidden" name="property_type" class="property_type"
                                                value="{{ @$hotel->propertytype->type }}">

                                            <?php
                                            if (isset($roomDetail)) {
                                                $bathroomDatas = explode(',', $roomDetail->bathroom_item);
                                            }
                                            ?>
                                            @foreach ($bathrooms as $bathroom)
                                                <div class="form-check py-3 border--dotted">
                                                    <label class="form-check-label para-fs-14 fs-6 w-100">
                                                        <input class="form-check-input" name="bathroom_item"
                                                            type="checkbox" value="{{ $bathroom->id }}"
                                                            {{ isset($roomDetail) && in_array($bathroom->id, $bathroomDatas) ? 'checked' : '' }}>
                                                        {{ $bathroom->item }}
                                                    </label>
                                                </div>
                                            @endforeach
                                ...
                                <div class="row justify-content-between">
                                    <div class="col-lg-6">
                                        <form action="" class="form-priceper-night">
                                            ...
                                            <div class="input-group mb-3 total-room-layout w-50">
                                                <label for="" class="form-label label-heading">Price for <span
                                                        class="total_person">1</span> person</label>
                                                <div class="input-group flex-nowrap">
                                                    <span class="input-group-text para-fs-14" id="addon-wrapping">INR/per
                                                        Night</span>
                                                    <input type="number"
                                                        class="form-control custom-from-control bed_price" placeholder="0"
                                                        value="{{ isset($roomDetail) ? exchange_rate($roomDetail->price_room) : '' }}">
                                                </div>
                                                <span id="bed_price_error" class="text-danger"></span>
                                            </div>
                                            <div class="amenities-title pt-3">
                                                <label for="" class="form-label label-heading ">Do you offer a
                                                    lower rate when there are less than 6 guests?</label>
                                            </div>
                                            <div class="amenities-raido-btn">
                                                <label class="form-check-label me-3" for="checkedyes"
                                                    style="display: initial;">
                                                    <div class="form-check form-check-inline amenities-radio me-0">
                                                        <input type="hidden" class="offerChecked"
                                                            value="{{ isset($roomDetail) && $roomDetail->discount == null ? 'no' : 'yes' }}">
                                                        <input class="form-check-input offer-check" type="radio"
                                                            name="flexRadioDefault" id="checkedyes" value="yes"
                                                            {{ isset($roomDetail) && $roomDetail->discount != 'null' ? 'checked' : '' }}>
                                                        Yes
                                                    </div>
                                                </label>
                                                <label class="form-check-label me-3" for="checkedno"
                                                    style="display: initial;">
                                                    <div class="form-check form-check-inline amenities-radio me-0">
                                                        <input class="form-check-input offer-check" type="radio"
                                                            name="flexRadioDefault" value="no" id="checkedno"
                                                            {{ isset($roomDetail) && $roomDetail->discount == null ? 'checked' : '' }}>
                                                        No
                                                    </div>
                                                </label>
                                            ...
                                            <div class="discount-div d-none">
                                                <div class="amenities-title pt-3 dicount-label-div">
                                                    <label for="" class="form-label label-heading ">How much
                                                        discount do you offer?</label>
                                                </div>
                                                <div class="input-group mb-3 discount-div ">
                                                    <div class="w-50 d-flex">
                                                        <input type="number" class="form-control discountValue"
                                                            ...
                                                            value="{{ isset($roomDetail) ? $roomDetail->discount : '0' }}"
                                                            min="1" max="100">
                                                        <select class="form-select c-form-select discountType"
                                                            style="border-radius: 0px" name="discount-type"
                                                            aria-label="Default select example">
                                                            <option value="percentage"
                                                                {{ isset($roomDetail) && $roomDetail->discount_type == 'percentage' ? 'selected' : '' }}>
                                                                %</option>
                                                            <option value="cash"
                                                                {{ isset($roomDetail) && $roomDetail->discount_type == 'cash' ? 'selected' : '' }}>
                                                                INR</option>
                                                        </select>
                                                    </div>
                                                ...
                                                <div class="p-form-select pt-3 price_wrapper offer-person-div">
                                                    <label for="" class="form-label label-heading">What is the
                                                        minimum occupancy you are willing to offer a discount for? </label>
                                                    <input type="hidden" class="numberOfpersonDiscount"
                                                        value="{{ isset($roomDetail) ? $roomDetail->min_person_discount : '' }}">
                                                    <select
                                                        class="form-select c-form-select star_rating offer-person-select"
                                                        name="star_rating" aria-label="Default select example">
                                                    </select>
                                                </div>
                                            </div>
                                        </form>
        ...
        $(document).ready(function() {
            ...
            function RoomDiscount(name) {
                ...
                for (i = 0; i < noOfPerson; i++) {
                    ...
                    $(".price-discount-div").append('<div class="row">' +
                        '<div class="col-6">' +
                        ' <p class="mb-1"><i class="fa-solid fa-user"></i> X <span class="number-person"></span>' +
                        j + '</p>' +
                        '</div>' +
                        '<div class="col-6">' +
                        '<p class="mb-1 ' + (amount <= 0 ? 'text-danger' : 'text-success') + '">Rs.' + amount +
                        '</p>' +
                        '</div>' +
                        '</div>');
            ...
            function selected(name) {
                ...
                for (i = 0; i < noOfPerson; i++) {
                    if (i != 0) {
                        ...
                        $('.offer-person-select').append('<option value="' + i + '"' + selected + '>' + i +
                            '</option>');
                    }
                }
            }
            ...
            $(".btn-submit-click").on('click', function() {
                ...
                !number_of_guest ? $(`#number_of_guest_error`).html(`Please tell us the number of guests`) :
                    $(`#number_of_guest_error`).html(``);

                let bed_price = $('.bed_price').val();
                !bed_price ? $(`#bed_price_error`).html(`Please enter a price`) : $(`#bed_price_error`)
                    .html(``);

                var BedDetail = $(".bed-option-div").map(function() {
                    return {
                ...
                let bathroom_item = $("input[name='bathroom_item']:checked").map(function() {
                    return $(this).val();
                }).get();
                    ...
                    success: function(data) {
                        ...
                        if (!$.isEmptyObject(data.roomlist)) {
                            $.each(data.roomlist[0].room_lists, function(index, roomlist) {
                                $('.room_name_select').append('<option value="' + roomlist.id +
                                    '">' + roomlist.room_name + '</option>');
                            })
                        }
                    }
                });
